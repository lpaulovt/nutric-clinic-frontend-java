...
export function DietPlanSection({}: DietPlanSectionProps) {
  ...
  const { data, isLoading } = useFindAllMealPlans(patient.id);
  ...
  return (
    <div className="flex flex-col gap-4">
      <Title title="Plano alimentar" underlineWidth="50%" />

      <div className="flex flex-col">
        {isLoading && <Spin />}
        {!isLoading &&
          data?.mealPlans.filter((item) => item.status === "ativo").length ===
            0 && <p>Não há planos cadastrados ainda.</p>}
        {!isLoading &&
          data?.mealPlans
